...
router.post('/rut-tien', function (req, res) {
    ...
    H_trade.find({ Phone_number: req.session.Phone_number }, function (err, docs) {
        if (docs) {
            H_trade.find({ Date: da }, function (err, docs1) {
                if (docs1) {
                    H_trade.find({ Type_trade: "rut tien" }, function (err, docs2) {
                        if (docs2 >= 2) {
                            ...
                            Wallet.find({ Phone_number: req.session.Phone_number }, function (err, docs) {
                                ...
                                if (Number(req.body.amount_money) > surplus) {
                                    res.render('rut-tien', { surplus: surplus, name: req.session.Fullname, error: "<div class='bg-red-100 rounded-lg py-5 px-6 text-base text-red-700 mb-3 text-center mt-3' role='alert'>Không đủ tiền</div>" })
                                }
                                else {
                                    if (req.body.card_number == "111111") {
                                        if (req.body.end_date != "2022-10-10") {
                                            res.render('rut-tien', { name: req.session.Fullname, error: "<div class='bg-red-100 rounded-lg py-5 px-6 text-base text-red-700 mb-3 text-center mt-3' role='alert'>Sai ngày</div>" })
                                        } else if (req.body.cvv != "411") {
                                            res.render('rut-tien', { name: req.session.Fullname, error: "<div class='bg-red-100 rounded-lg py-5 px-6 text-base text-red-700 mb-3 text-center mt-3' role='alert'>Sai CVV</div>" })
                                        } else {
                                            console.log(check)
                                            Wallet.find({ Phone_number: req.session.Phone_number }, function (err, docs) {
                                                if (docs) {
        ...
        res.render('rut-tien',{error:"Thành công"})
    })
})

router.get('/test', function (req, res) {
    let docs = H_trade.find({ Phone_number: "0562413183" })
    console.log(docs)
})
